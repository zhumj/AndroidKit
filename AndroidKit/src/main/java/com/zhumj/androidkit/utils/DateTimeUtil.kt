...
object DateTimeUtil {
    ...
    fun getLocalDateTime(context: Context?, timeInMillis: Long): String? {
        return DateUtils.formatDateTime(context, timeInMillis, DateUtils.FORMAT_SHOW_TIME or DateUtils.FORMAT_SHOW_DATE or DateUtils.FORMAT_SHOW_YEAR)
    }
    ...
    fun getFormat(format: String): SimpleDateFormat = SimpleDateFormat(format, Locale.getDefault())
    ...
    fun getNowDateFormat(format: String): String {
        ...
        return getFormat(format).format(currentTime)
    }
    ...
    fun getDateFormat(dateString: String, format: String): String {
        return try {
            val simpleDateFormat = getFormat(format)
    ...
    fun getAgeByBirthday(birthday: String): Int {
        try {
            ...
            calendar.time = getFormat(DATE_FORMAT_2).parse(birthday)!!
